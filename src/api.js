...
const BACKEND_DOMAIN = (import.meta.env.VITE_BACKEND_URL || DEFAULT_NGROK_DOMAIN).replace(/\/$/, '');

// On localhost development server, use relative path '' so Vite dev proxy handles ngrok headers.
// On production deployments (e.g. Vercel), call BACKEND_DOMAIN directly to bypass Vercel reverse proxy 404 issues.
const isLocalhost = typeof window !== 'undefined' &&
  (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1');

export const API_BASE_URL = isLocalhost ? '' : BACKEND_DOMAIN;
